    ...
    <body class="min-h-screen bg-white dark:bg-zinc-800">
        ...
        <laraliveui:header class="lg:hidden">
            ...
            <laraliveui:dropdown position="top" align="end">
                <laraliveui:profile :initials="auth()->user()->initials()" icon-trailing="chevron-down" />
                <laraliveui:menu>
                    <laraliveui:menu.radio.group>
                        <div class="p-0 text-sm font-normal">
                            <div class="flex items-center gap-2 px-1 py-1.5 text-start text-sm">
                                <laraliveui:avatar :name="auth()->user()->name" :initials="auth()->user()->initials()" />
                    ...
                    <form method="POST" action="{{ route('logout') }}" class="w-full">
                        @csrf
                        <laraliveui:menu.item as="button" type="submit" icon="arrow-right-start-on-rectangle" class="w-full cursor-pointer" data-test="logout-button">
                            {{ __('Log Out') }}
                        </laraliveui:menu.item>
                    </form>
                </laraliveui:menu>
